...
def main(cg, ipc, means, code_directory, clase):
    ...
    """
    path = '../../results/'+code_directory+'_'+clase+'/signal_reconstruction/plot/'
    check_path(path)
    instructions_per_path_df = get_instructions_per_path(cg, code_directory, clase)
...
def get_instructions_per_path(cg, code_directory, clase):
    """ Reads the data generated by the instruction estimation module and retrieves
    the number of instructions per path
    """
    directory = '../../results/'+code_directory+'_'+clase+'/instructions_estimation/instructions_per_path_SP_D.csv'
    indexes = []
    instructions = []
...
def generate_plot(x_axis, values, counter, path, code_directory, clase):
    """ Creates a plot with the data contained on the arguments
    """
    plt.figure(figsize=(9,4))
    plt.plot(x_axis, values, label=counter)
    plt.title(code_directory+ ' - CLASE '+clase+' (Reconstrucción de la señal del contador hardware)')
    plt.xlabel('Tiempo(s)')
    plt.legend()
